...
async function ensureIndexes(db) {
  ...
  await db.collection('schedule_courses').createIndex({ course_id: 1 }, { unique: true })
  await db.collection('schedule_courses').createIndex({ code: 1 }, { unique: true })
  await db.collection('schedule_sections').createIndex({ section_id: 1 }, { unique: true })
  await db.collection('schedule_sections').createIndex({ course_id: 1, name: 1 }, { unique: true })
  await db.collection('rooms').createIndex({ room_id: 1 }, { unique: true })
  await db.collection('rooms').createIndex({ section_id: 1, name: 1 }, { unique: true })
  await db.collection('labs').createIndex({ lab_id: 1 }, { unique: true })
  await db.collection('labs').createIndex({ room_id: 1, name: 1 }, { unique: true })
  await db.collection('labs').createIndex({ faculty_user_id: 1 })
}
